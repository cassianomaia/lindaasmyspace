...
#cria o server, basicamente o computador que chama essa função torna-se o servidor
...
class ServerThread(Thread):
    ...
    #inicializando a classe thread server
    ...
    def threaded(self, conn):
        while True:
            ...
            data = conn.recv(1024)  # recebendo dados do cliente
            if not data: break  # para caso o cliente parar
    ...
    def run(self):
        while True:
            self.sck.listen(1)
            (self.conn, self.addr) = self.sck.accept()  # retorna um novo socket e um cliente
            print("Cliente conectado.")
            start_new_thread(self.threaded, (self.conn,))
            print("Thread criada pelo usuário")
